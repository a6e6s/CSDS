                        ...
                        <form action="{{ route('contacts.index') }}" method="get" id="search-form">
                            ...
                            <th class="p-1">
                                <select class="form-select form-select-sm" name="search[status]">
                                    ...
                                    <option {{ optional(request('search'))['status'] == 1 ? 'selected' : '' }} value="1">@lang('Unreaded')</option>
                                    <option {{ optional(request('search'))['status'] == '0' ? 'selected' : '' }} value="0">@lang('Readed')</option>
                                </select>
                            </th>
                            <th class="p-1"></th>
                            <th class="p-1">
                                        ...
                                        @if ($contact->status)
                                            <li>
                                                <form action="{{ route('contacts.actions', ['id' => $contact->id]) }}" method="post">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" name="block" value="1" class="dropdown-item"><i class="icofont-ban text-warning pe-3"></i>@lang('Readed')</button>
                                                </form>
                                            </li>
                                        @else
                                            <li>
                                                <form action="{{ route('contacts.actions', ['id' => $contact->id]) }}" method="post">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" name="activate" value="1" class="dropdown-item"><i class="icofont-check-circled text-success pe-3"></i>@lang('Unreaded')</button>
                                                </form>
                                            </li>
                                        @endif
                                        <li>

...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><span>@lang('Doctor')</span></li>
@endsection
...
@section('content')
    <div class="row">
        <div class="text-end pb-2">
            <a href="{{ route('doctors.create') }}" class="col btn btn-sm btn-primary"><i class="icofont-plus"></i> @lang('Add New')</a>
        </div>
    </div>
    <div class="card">
        <div class="card-header bg-white h5">
            <i class="icofont-chart-flow text-dark"></i> @lang('Doctor')
        </div>
        <div class="card-body">
                ...
                <tbody>
                    <tr class="d-none actionRow">
                        <td colspan="8" class="text-start bg-white">
                            <form action="{{ route('doctors.actions') }}" method="post" id="multi-actions-form">
                                @csrf @method('PATCH')
                    ...
                    @forelse ($doctors as $doctor)
                        <tr>
                            <td class="text-start"><input name="record[]" class="form-check-checkbox me-2" onclick="isChecked()" type="checkbox" value="{{ $doctor->id }}"> </td>
                            <td class="text-start">{{  $doctor->name }}</td>
                            <td> {{ ucfirst($doctor->email) }} </td>
                            <td> {{ ucfirst($doctor->specialty->name) }} </td>
                            <td> {!! $doctor->status ? '<i class="icofont-check-circled text-success icofont-lg"></i>' : '<i class="icofont-ban icofont-lg text-danger"></i>' !!}</td>
                            <td> {{ optional($doctor->created_at)->format('d-m-Y') }} </td>
                            <td>
                                <div>
                                    ...
                                    <ul class="dropdown-menu p-0 ltr">
                                        {{-- <li><a class="dropdown-item" href="{{ route('doctors.show', ['doctor'=>$doctor->id]) }}"><i class="icofont-eye-alt text-info pe-3"></i>@lang('Show')</a></li> --}}
                                        <li><a class="dropdown-item" href="{{ route('doctors.edit', ['doctor' => $doctor->id]) }}"><i class="icofont-edit text-info pe-3"></i>@lang('Edit')</a></li>
                                        @if ($doctor->status)
                                            <li>
                                                <form action="{{ route('doctors.actions', ['id' => $doctor->id]) }}" method="post">
                                                    @csrf @method('PATCH')
        ...
        <div class="card-footer text-muted d-flex pb-0">
            <div class="col-6">@lang('Total') : {{ $doctors->total() }}</div>
            <div class="col-6">{{ $doctors->appends(Request::all())->links() }}</div>
        </div>
    </div>
@endsection
@section('script')

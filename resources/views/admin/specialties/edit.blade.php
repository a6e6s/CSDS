...
@section('content')
    ...
    <form action="{{ route('specialties.update', $specialty) }}" method="post" class="row" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div class="input-group mb-3">
                            <input class="form-control" value="{{ $specialty->name }}" name="name" type="text"
                    ...
                    <div class="mb-3">
                        <label for="logo" class="form-label">@lang('Specialties logo')</label>
                        <input type="file" class="form-control @error('logo') is-invalid @enderror" id="logo"
                            name="logo">
                        @if ($specialty->logo)
                            <img salt="logo" width="100px"
                                src="{{ asset('storage/images/specialties/' . $specialty->logo) }}">
                        @endif
                        @error('logo')
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group mb-3">
                            <textarea class="form-control" name="description" type="text" placeholder="@lang('Specialties description')"
                                id="description">{{ $specialty->description }}</textarea>
                            @error('description')
                                <span class="break p-2 text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div class="form-check mb-3">
                            <input {{ $specialty->status == '0' ? 'checked' : '' }} class="btn-check" id="blocked"
                            ...
                            <input {{ $specialty->status != '0' ? 'checked' : '' }} class="btn-check" id="active"

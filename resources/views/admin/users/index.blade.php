...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><span>@lang('User')</span></li>
@endsection
...
@section('content')
    <div class="row">
        <div class="text-end pb-2">
            <a href="{{ route('users.create') }}" class="col btn btn-sm btn-primary"><i class="icofont-plus"></i> @lang('Add New')</a>
        </div>
    </div>
    <div class="card">
        <div class="card-header bg-white h5">
            <i class="icofont-chart-flow text-dark"></i> @lang('User')
        </div>
        <div class="card-body">
                ...
                <tbody>
                    <tr class="d-none actionRow">
                        <td colspan="8" class="text-start bg-white">
                            <form action="{{ route('users.actions') }}" method="post" id="multi-actions-form">
                                @csrf @method('PATCH')
                            ...
                            <td>
                                <div>
                                    ...
                                    <ul class="dropdown-menu p-0 ltr">
                                        {{-- <li><a class="dropdown-item" href="{{ route('users.show', ['user'=>$user->id]) }}"><i class="icofont-eye-alt text-info pe-3"></i>@lang('Show')</a></li> --}}
                                        <li><a class="dropdown-item" href="{{ route('users.edit', ['user' => $user->id]) }}"><i class="icofont-edit text-info pe-3"></i>@lang('Edit')</a></li>
                                        @if ($user->status)
                                            <li>
                                                <form action="{{ route('users.actions', ['id' => $user->id]) }}" method="post">
                                                    @csrf @method('PATCH')
        ...
        <div class="card-footer text-muted d-flex pb-0">
            <div class="col-6">@lang('Total') : {{ $users->total() }}</div>
            <div class="col-6">{{ $users->appends(Request::all())->links() }}</div>
        </div>
    </div>
@endsection
@section('script')

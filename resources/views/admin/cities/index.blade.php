...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><span>@lang('Cities')</span></li>
@endsection
...
@section('content')
    <div class="row">
        <div class="text-end pb-2">
            <a href="{{ route('cities.create') }}" class="col btn btn-sm btn-primary"><i class="icofont-plus"></i> @lang('Add New')</a>
        </div>
    </div>
    <div class="card">
        <div class="card-header bg-white h5">
            <i class="icofont-chart-flow text-dark"></i> @lang('Cities')
        </div>
        <div class="card-body">
            <table class="table table-responsive-lg table-bordered table-striped table-bordered text-center" id="datatable">
                <thead class="thead-inverse">
                    ...
                    <tr class="search-row">
                        <form action="{{ route('cities.index') }}" method="get" id="search-form">
                            <th class="p-1"></th>
                ...
                <tbody>
                    <tr class="d-none actionRow">
                        <td colspan="7" class="text-start bg-white">
                            <form action="{{ route('cities.actions') }}" method="post" id="multi-actions-form">
                                @csrf @method('PATCH')
                    ...
                    @forelse ($cities as $city)
                        <tr>
                            <td class="text-start"><input name="record[]" class="form-check-checkbox me-2" onclick="isChecked()" type="checkbox" value="{{ $city->id }}"> </td>
                            <td class="text-start">{{  $city->name }}</td>
                            <td> {!! $city->status ? '<i class="icofont-check-circled text-success icofont-lg"></i>' : '<i class="icofont-ban icofont-lg text-danger"></i>' !!}</td>
                            <td> {{ optional($city->created_at)->format('d-m-Y') }} </td>
                            <td>
                                <div>
                                    ...
                                    <ul class="dropdown-menu p-0 ltr">
                                        {{-- <li><a class="dropdown-item" href="{{ route('cities.show', ['city'=>$city->id]) }}"><i class="icofont-eye-alt text-info pe-3"></i>@lang('Show')</a></li> --}}
                                        <li><a class="dropdown-item" href="{{ route('cities.edit', ['city' => $city->id]) }}"><i class="icofont-edit text-info pe-3"></i>@lang('Edit')</a></li>
                                        @if ($city->status)
                                            <li>
                                                <form action="{{ route('cities.actions', ['id' => $city->id]) }}" method="post">
                                                    @csrf @method('PATCH')
        ...
        <div class="card-footer text-muted d-flex pb-0">
            <div class="col-6">@lang('Total') : {{ $cities->total() }}</div>
            <div class="col-6">{{ $cities->appends(Request::all())->links() }}</div>
        </div>
    </div>
@endsection
@section('script')
